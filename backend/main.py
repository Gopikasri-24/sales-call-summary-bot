...
# sum_model_id = "sshleifer/distilbart-cnn-12-6"
...
# # 3. Whisper Audio Pipeline 
# transcriber = pipeline("automatic-speech-recognition", model="openai/whisper-tiny", chunk_length_s=30)
...
#     needs = list(dict.fromkeys(needs)) # Remove duplicates
...
#     # --- NEW: Music/Noise Filter ---
#     words = text.split()
#     if len(words) < 15 or "[Music]" in text:
#         raise HTTPException(status_code=400, detail="Audio rejected: No valid speech detected. Ensure this is a conversation, not music or background noise.")
...
#     summary_ids = sum_model.generate(inputs["input_ids"], max_length=max_len, min_length=min_len, do_sample=False, early_stopping=True)
#     summary = sum_tokenizer.decode(summary_ids[0], skip_special_tokens=True)

#     # --- NEW: Advanced Sentiment Logic (Averages the whole call) ---
#     chunk_size = 400
...
#     for chunk in chunks:
#         res = sentiment_analyzer(chunk[:512])[0]
...
#         sentiment = f"NEGATIVE ({round((total_neg / (total_neg+total_pos))*100, 2)}%)"
#     else:
#         sentiment = f"POSITIVE ({round((total_pos / (total_neg+total_pos))*100, 2)}%)"
...
#     cursor = conn.cursor()
#     cursor.execute(
#         "INSERT INTO history (transcript, summary, sentiment, action_items, customer_needs) VALUES (?, ?, ?, ?, ?)",
...
#     # Text processor handles the music validation check
...
#     conn = sqlite3.connect("sales_bot.db")
#     cursor = conn.cursor()
#     cursor.execute("SELECT id, summary, sentiment FROM history ORDER BY id DESC LIMIT 5")
#     rows = cursor.fetchall()
#     conn.close()
#     return [{"id": r[0], "summary": r[1], "sentiment": r[2]} for r in rows]
...
async def process_text(input_data: TextInput):
    ...
    if total_neg > total_pos:
        sentiment = (
            f"NEGATIVE "
            f"({round((total_neg/(total_pos+total_neg))*100,2)}%)"
        )
    else:
        sentiment = (
            f"POSITIVE "
            f"({round((total_pos/(total_pos+total_neg))*100,2)}%)"
        )
    ...
    customer_needs_str = "\n".join(
        f"- {item}" for item in customer_needs
    )
